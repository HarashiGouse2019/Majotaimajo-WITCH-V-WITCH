...
public class TargetDetection : BaseBulletBehaviour {
	...
	public override void OnBulletBirth ()
	{
		...
		if (mainTargetObj != null && distance <= detectionRange)
			targetVector = (mainTargetObj.targetTransform.position - bullet.transform.position).normalized;
		else
		...
		bullet.transform.rotation = (mainTargetObj != null) ?
			Quaternion.Euler(0f, 0f, angle + 270f) :
			Quaternion.Euler(Vector3.zero);

		Debug.Log(mainTargetObj != null ? "Found" : "No Target Obj");
	}
	...
	private void DetectDistanceFromTarget()
	{
		...
		{
			distance = (target.targetTransform.position - defaultParent.transform.position).magnitude;
